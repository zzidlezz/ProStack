...
class PGE(nn.Module):
    ...
    def forward(self, x, edge_index,xxx,inference=False):
        if self.args.dataset == 'ogbn-products':
            edge_index = self.edge_index
            n_part = 5
            splits = np.array_split(np.arange(edge_index.shape[1]), n_part)
        ...
        adj = edge_embed.reshape(xxx, xxx)
    ...
    @torch.no_grad()
    def inference(self, x,edge_index,xxx):
        # self.eval()
        adj_syn = self.forward(x,edge_index,xxx, inference=True)
        return adj_syn
