...
#gnn
parser.add_argument('--nlayers', type=int, default=4)           #4   0.25:8320  0.5 8220   1 0.8220    default 2
...
parser.add_argument('--teacher_model', type=str, default='SGC_Multi')#arxiv:SGC other:SGC_Multi
...
parser.add_argument('--lr_model', type=float, default=0.0001)  #0.0001
parser.add_argument('--lr_teacher_model', type=float, default=0.00001)##arxiv:0.01 products:0.001 cora/citeseer:0.00001
...
parser.add_argument('--student_model_loop', type=int, default=3000)  #3000
parser.add_argument('--teacher_val_stage', type=int, default=50)
parser.add_argument('--student_val_stage', type=int, default=100)  #100
...
def train_teacher():
    ...
    for it in range(args.teacher_model_loop+1):
        ...
        # loss = loss_func(output[idx_train], labels_train[idx_train])
        ...
        if(it%args.teacher_val_stage==0):
            ...
            # acc_train1 = eval1(output[idx_train], labels_train)
            # acc_val1 = eval1(output[idx_val], labels_val)
            # acc_test1 = eval1(output[idx_test], labels_test)
            # print(f'Epoch: {it:02d}, '
            #       f'Loss: {loss.item():.4f}, '
            #       f'Train: {100 * acc_train1:.4f}%, '
            #       f'Valid: {100 * acc_val1:.4f}% '
            #       f'Test: {100 * acc_test1:.4f}%')

            if(acc_val>best_val):
                best_val=acc_val
                best_test=acc_test
                if args.save:
                    torch.save(teacher_model.state_dict(), f'{root}/saved_model1/teacher/{args.dataset}_{args.teacher_model}_{args.seed}.pt')
...
def train_syn():
    ...
    for interval_idx in range(0, 4):
        ...
        if previous_images is not None:
            # previous_images = previous_images.to(feat_syn1.device)
            # previous_labels = previous_labels.to(labels_syn1.device)
            ...
            with torch.no_grad():
                ...
                # feat_syn = previous_images
                # labels_syn = previous_labels
            # if interval_idx == 1 or 3:
            #     # args.lr_model *= 0.1
            #     args.lr_feat = 0.05
                # args.lr_adj *= 0.1
        ...
        for i in range(args.condensing_loop+1):
            ...
            if i >= 100 and i % 100 == 0:
                ...
                # teacher_output_syn = teacher_model._evaluate_syn(feat_syn, edge_index_syn, edge_weight=edge_weight_syn)
                    ...
                    if j%args.student_val_stage==0:
                        ...
                        if(acc_val>best_val):
                            ...
                            if args.save:
                                ...
                                torch.save(validation_model.state_dict(), f'{root}/saved_model1/student/{args.dataset}_{args.teacher_model}_{args.validation_model}_{args.reduction_rate}__{interval_idx}_3_256_0.5_relu_1.pt')
...
if __name__ == '__main__':
    ...
    # else:
    #     if args.dataset in ["cora"]:
    teacher_model = SGC_Multi_PYG(nfeat=d, nhid=2048, nclass=nclass, dropout=0.25, K=args.nlayers, nlayers=2, norm=None).to(device)  # dropout=0

        # if args.dataset in ["citeseer"]:
        #     teacher_model = SGC_Multi_PYG(nfeat=d, nhid=2048, nclass=nclass, dropout=0.25, K=args.nlayers, nlayers=2,norm=None).to(device)
        #
        # else:
    ...
    if not os.path.exists(root+'/saved_model1/teacher/'+args.dataset+'_'+args.teacher_model+'_'+str(args.seed)+'.pt'):
        print("Traning Teacher!")
        train_teacher()
    teacher_model.load_state_dict(torch.load(f'{root}/saved_model1/teacher/{args.dataset}_{args.teacher_model}_{args.seed}.pt'))
    ...
    if args.alignment == 1 and args.smoothness == 1:
        # if not os.path.exists(root+'/saved_ours1/feat_'+args.dataset+'_'+args.teacher_model+'_'+args.validation_model+'_'+str(args.reduction_rate)+'_'+str(args.seed)+'.pt'):
        #     print("Condensing!")
        #     train_syn()
    ...
    if args.model=='GCN':
        ...
        model = MLP_PYG(channel_list=[d, 2048, nclass], in_channels=d, hidden_channels=args.hidden, out_channels=nclass, dropout=[args.dropout, args.dropout, args.dropout, args.dropout], num_layers=args.nlayers, norm=None, act=None).to(device)
    model.initialize()
    optimizer=optim.Adam(model.parameters(), lr=args.lr_model)

    # model1.initialize()
    # optimizer1 = optim.Adam(model1.parameters(), lr=args.lr_model)
    ...
    for j in range(args.student_model_loop):
        ...
        if args.model!='MLP':
            ...
            # output_syn = model.forward(feat_syn, edge_index_syn, edge_weight=edge_weight_syn)[selected_indices]
        ...
        if j%args.student_val_stage==0:
            ...
            if(acc_val>best_val):
                ...
                best_test=acc_test
                if args.save:
                    torch.save(model.state_dict(), f'{root}/saved_model1/student/{args.dataset}_{args.teacher_model}_{args.model}_{args.reduction_rate}_{args.nlayers}_{args.hidden}_{args.dropout}_{args.activation}_{args.seed}.pt')
