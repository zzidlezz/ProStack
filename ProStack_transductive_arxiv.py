...
def train_teacher():
    ...
    print('Pre Duration:', round(end - start), 's')
...
def train_syn():
    ...
    if args.validation_model=='GCN':
        validation_model = GCN_PYG(nfeat=d, nhid=args.hidden, nclass=nclass, dropout=args.dropout, nlayers=args.nlayers, norm='BatchNorm', act=None).to(device)
    ...
    for interval_idx in range(0,10):
        ...
        if interval_idx == 0:
            current_condensing_loop = 1500  # 第一个 interval 使用原始值
        else:
            current_condensing_loop = 800

        for i in range(current_condensing_loop+1):
...
if __name__ == '__main__':
    ...
    if not os.path.exists(root+'/saved_ours1/feat_'+args.dataset+'_'+args.teacher_model+'_'+args.validation_model+'_'+str(args.reduction_rate)+'_'+str(args.seed)+'_'+{9}+'.pt'):
        print("Condensing!")
        train_syn()


    # feat_syn = torch.load(f'{root}/saved_ours/feat_{args.dataset}_{args.teacher_model}_{args.validation_model}_{args.reduction_rate}_{args.seed}_{9}.pt').to(device)
    # pge.load_state_dict(torch.load(f'{root}/saved_ours/pge_{args.dataset}_{args.teacher_model}_{args.validation_model}_{args.reduction_rate}_{args.seed}_{9}.pt'))
    # labels_syn =torch.load(f'{root}/saved_ours/labels_{args.dataset}_{args.teacher_model}_{args.validation_model}_{args.reduction_rate}_{args.seed}_{9}.pt')
    ...
    if args.model=='GCN':
        ...
        model = GCN_PYG(nfeat=d, nhid=args.hidden, nclass=nclass, dropout=args.dropout, nlayers=args.nlayers, norm='BatchNorm', act=None).to(device)
